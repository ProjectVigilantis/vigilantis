...
> **상태: 본문 8장 완성 · PM 리뷰 반영 · 재리뷰 대기** — 2026-10-01(목) 중간 발표(Connect Day 1일차) 제출용. 이슈 #335.
...
**그리고 전담을 두지 않는 쪽이 다수다.** 가장 가까운 공개 통계는 이것이다.

| | 값 | 뜻 |
| --- | --- | --- |
| 사이버보안 담당 인력 중 **겸업** 비율 | **63.8%** | 2024년 기준 — 본업이 따로 있고 보안을 얹어서 한다 |

> **인용 출처** — 과학기술정보통신부 「2024년 사이버보안 인력수급 실태조사」. **원 보고서를 직접 열지 못해 보도(이투데이 2025-04-10)로 확인한 값이다.**

**이 수치를 우리 주장으로 그대로 옮기지는 않는다.** 두 가지가 다르다 — ① **직무**가 보안이고 인프라가 아니다 ② **분모**가 기업 수가 아니라 **인력 수**다("담당하는 인력 중 63.8%"). 같은 조사의 외부 인력 7.8%도 분모가 같다고 확인되지 않아 **두 값을 더하지 않는다.** 인프라 직무의 같은 조사는 찾지 못했다. **방향은 같지만 그대로 옮겨 적을 수는 없는 근거**로 둔다 — 값과 단위가 어긋난 인용은 질문 한 번에 문서 전체의 신뢰를 깎는다.
...
| **유휴 자산** | **월 40만 원**(참고치) | 자산 20대 중 저활성 4대를 **정리(정지·삭제)** 했을 때 — AWS 서울 리전 온디맨드 기준. **이 도구의 조치 범위 밖이다**(아래) |

> **수치 기준** — 원티드 DevOps 직무 연봉 데이터(2025) · AWS 서울 리전 온디맨드 요금. **유휴 자산 값은 추정 참고치이며 실적이 아니다.**

**두 값의 성격이 다르고, 뒤의 것은 이 도구가 만드는 값이 아니다.**

- **인건비 6,000만 원**은 **「피하는 비용」**이다 — 이 도구가 없으면 사람을 뽑거나, 뽑지 않고 겸업 담당자가 떠안는다(§1.2.2).
- **유휴 자산 40만 원**은 **「정리했을 때의 참고치」**다. **Action Whitelist에는 EC2를 정지·삭제하는 런북이 없다**(§2.2) — 유휴 EC2에 대한 조치는 `RIGHTSIZING` 하나이고, 같은 패밀리 안에서 **메모리 1/4 이상 · 2 GiB 하한**을 지키는 크기까지만 내린다(예: `t3.xlarge → t3.medium`이고 `m5.large`는 후보가 없다). **그래서 이 도구가 실제로 줄이는 금액은 위 참고치보다 작다.** 실제 절감은 조치 건마다 계산해 승인 화면에 올린다(§2.1 AI 계층).
