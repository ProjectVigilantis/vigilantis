...
**그리고 그런 회사가 빠르게 늘고 있다.**

| | 값 | 뜻 |
| --- | --- | --- |
| 국내 SaaS 기업 수 | **1,642개** | 2023년 기준 · **전년 대비 +24%** |

> **인용 출처** — 과학기술정보통신부 「2024 클라우드산업 실태조사」
...
<!-- 시장 규모·성장률 수치(99.5억→382.8억 달러 · CAGR 27.2% · 정부 목표 5조→10조)는 2026-09-17에 이 절에서 뺐다. 9/4 제출본 §04 슬라이드에 기관명이 없었고(실측), 작성자(김세혁)도 원출처를 특정하지 못했다. 같은 슬라이드에서 출처가 붙어 있던 1,642개(+24%)만 남겼다. 나중에 시장 규모 수치를 다시 넣으려면 기관명·조사명·연도를 함께 확보한 뒤 넣는다 — 검색으로 즉시 나온 숫자는 그 자체로 출처가 아니다(배점 ⑦ 문서화·출처). -->
...
| 6 | 조치별 **절감 예상 금액과 그 산출 근거**를 함께 제시한다 | 저장된 근거 필드 조회 | 🔶 **금액·근거 생성·저장·조회 v1 머지**(2026-09-17) — 화면 표시까지의 잔여는 #347 |
...
**이 표의 날짜가 프로젝트의 고정점이다.** 다른 문서·이슈가 다른 날짜를 말하면 이 표가 이긴다.

| 날짜 | 무엇 | 성격 | 범위 |
| --- | --- | --- | --- |
| 2026-09-04(금) ✅ 완료 | 기획 발표 | 발표 | 기획·문서 단계까지. 구현 시연 없음 |
| 2026-09-30(수) | E2E 시연물 완성 마감 | 내부 마감 | 10/1(목)에 시연할 경로가 **이날 전부 서 있어야 한다.** 발표 자리가 아니다 |
| **2026-10-01(목)** | **중간 발표 — MVP 시연** | **발표·평가** | Connect Day 1일차 세션. 런북 10종·양방향 회복·4단계 가드레일을 시연하며 **시연 환경은 LocalStack이다**(§4.4). 제출물로 **이 기획서 1부**가 함께 나간다 |
...
| 8주차 | 9/28(월)–**10/01(목)** · 실작업 **4일** | 릴리스 컷 → 리허설 → **중간 발표** | 9/28(월) `main` 릴리스 컷·태그 → 9/29(화)–9/30(수) 리허설 → **9/30(수) E2E 시연물 완성 마감** → **10/1(목) 중간 발표(Connect Day 1일차) · 기획서 제출** | 릴리스 컷·운영 머신·환경 **김세혁** / 대본 진행·판정 기록 **김승철** (2026-09-16 확정 · 박지현 부재) |
| 9주차 | 10/02(금)–10/08(목) · 4일 (스모크 실작업 **3일**) | **실 AWS 스모크** — ADR-0006 §4 이월 목록 해소 | 전용 VPC·전용 키·비용 통제 구성(ADR-0009)과 스모크 실행 | 김세혁(`INFRA`) — 10/2(금) Connect Day 2일차 · **10/5(월) 개천절 대체 휴일** · 10/6(화) 업무 재배정 · **스모크 10/6(화)–10/8(목)** |
...
2. §1.2 시장 수치 원출처(조사기관·보고서명) — 김세혁 확인 대기. 확인되면 91행 TODO 주석을 지우고 표 아래에 명시한다.
-->
