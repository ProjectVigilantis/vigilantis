...
>
> **출처 표기 규칙** — 각 절 머리에 `> **출처** — …` 한 줄로 그 절의 값이 어디서 왔는지 밝힌다. 원천은 셋뿐이다: **SSOT**(범위·결정·역할·현황) · **`docs/adr/`**(설계 결정 배경) · **`packages/schemas/`**(계약의 코드 소재). 저장소 밖 문서는 원천으로 쓰지 않는다 — 대조할 수 없는 기준은 기준이 아니다.
...
클라우드 비용 최적화 도구도, 보안 탐지 도구도 이미 많다. **이 프로젝트가 다르게 잡은 것은 「탐지 다음」이다.**
