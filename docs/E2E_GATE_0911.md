...
> **④가 앞의 셋과 다른 점**: ①③은 **코드가 없어서** 막힌 것이라 9/11까지 못 푼다. ②는 우회가 서 있다. **④만 당일 전에 풀 수 있고, 그래서 제일 급하다** — 환경변수 한 줄이면 되지만 키를 제가 갖고 있지 않습니다(#316). **9/10까지 결론이 나야 한다** — 풀리면 T1 이 실경로 6컷으로 서고, 안 풀리면 대체 컷으로 확정해 §0·§3-1·§7 ⓒ 를 고친다. **미정으로 당일을 맞지 않는다.**
