...
| 6 | 실행 | 진행 표시 | `ec2.modify_instance_attribute` | 시작 이벤트 없음 — 판정 결과 확정 시 발행 | LocalStack 재기동 후 재시도 |
...
| 9 | 원복 완료 | **AST-001로 이동해** 인스턴스 유형 복귀 확인 | 자식 `SUCCESS` → 원본 Execution `ROLLED_BACK`(함께 확정) | `EXECUTION_UPDATED` **자식 `SUCCESS`**<br>`INCIDENT_UPDATED` **Incident `AWAITING_CLOSURE`** | 관제자 [종료 판단]이 남는다 |
...
- **승인은 한 번뿐이다(카드와 확인 모달에서 실행 클릭 2회).** 5번의 ACT-001 [실행] 이후 사람은 아무것도 누르지 않는다. 8–9번은 전부 시스템이 한다.
...
**한 줄**: 한 IP가 SSH를 두드려대는 걸 잡아 그 주소만 핀셋으로 막고, 관제자가 확인한 뒤 한 번 승인으로 되돌린다(상세 버튼 → 확인 모달, 실행 클릭 2회).
...
| 5 | **관제자 승인 → 차단** | **INC-002 상세 [승인하고 차단] → ACT-001 [실행]** | `RUNBOOK_NACL_ADD_DENY`<br>`trigger_source: USER_APPROVAL`<br>`approval_mode: HUMAN_ONLY`<br>`ec2.create_network_acl_entry` | `EXECUTION_UPDATED` `SUCCESS` | — |
| 6 | 관제자 확인 | 상세에서 **판단 근거** 확인 | `GET /api/v1/incidents/{id}` | — | — |
| 7 | **원클릭 해제** | **INC-002 상세 [승인하고 해제] → ACT-001 [실행]** | `RUNBOOK_NACL_RESTORE`<br>`trigger_source: USER_APPROVAL` | `EXECUTION_UPDATED` | **핵심 컷** |
...
`RUNBOOK_NACL_RESTORE`는 **본편 7종**이라 `ExecutionSummary.available_recovery_runbook_ids`로 올 수 없다 — 그 필드는 validator가 **롤백 3종만** 허용한다. 따라서 [해제] 버튼은 **`recommendations`** 로 렌더된다. 이 경로는 `proposalRequest()`의 `variant: ACTION`이므로 ACT-001의 버튼은 **[실행]**이다. 복구 계열(`RECOVERY`)의 [해제 실행]과 구분한다.
...
**위협 관측부터 해제까지 LocalStack에서 끝까지 갔고, 관제자 승인 요청은 [조치 실행] 2건이었다.** HTTP를 `TestClient`로 보낸 기록이므로 브라우저 클릭 횟수의 실측은 아니다. 현재 화면에서는 승인 2건에 **실행 클릭 4회**(각 화면 버튼 → ACT-001 [실행])가 필요하며, 화면 이동·필터 조작은 이 수에 포함하지 않는다. 입력은 S3이고, 대상만 시드 인스턴스 `vigilantis-seed-idle`로 바꿨다(`--target-arn`) — 그 서브넷에 시드 NACL이 연결돼 있어 `PROTECTED_BY` 관계가 선다.
...
> **기준**: LocalStack 시드 환경(`scripts/seed_localstack.py`) — 10/1(목) 시연은 LocalStack 기반이다(SSOT §확정 결정 로그 2026-09-14). **측정**: 2026-09-17, 시드 → 수집·판정 1회(`services.scheduler.run_pipeline`, 일회용 DB) — 앱 기동·모델 호출 없이. **T1-3은 같은 날 따로 쟀다** — 같은 시드·일회용 DB에서 스캔 1회 → AI 분석 스캔 1회(`agent_dispatcher.dispatch_pending_analysis` · 실 모델 호출 · `gpt-5.6-luna` · reasoning `low` — 기본 설정)(§결정 기록 ④). 실행 단계(T1 5–9번 · T2 4–8번)는 PR #346 실측(T1)과 §T2 관통 실측(2026-09-15)을 근거로 한다. **결정**: PR #371 리뷰(김세혁, 2026-09-17). **담당**: 김승철(#356 ③ · 기한 9/23(수)).
>
...
docker compose exec api uv run python -c "import os; from openai import OpenAI; OpenAI(max_retries=0).models.retrieve(os.getenv('OPENAI_MODEL') or 'gpt-5.6-luna'); print('OK')"
```

마지막 줄은 **키 확인**이다(제안 · PR #376 리뷰). 앞에 `uv`의 동기화 출력이 붙으므로 **마지막 줄이 `OK`인지** 본다(2026-09-18 컨테이너 안에서 확인). 아니면 카드가 생기기 전에 멈춰 키를 고치고 `docker compose up -d --force-recreate api`만 하면 된다 — 사전 준비를 처음부터 할 필요가 없다. 모델 조회라 토큰을 쓰지 않고 모델 접근 권한까지 본다. 실패 경로(잘못된 키 → 종료 코드 1)는 PR #376 리뷰에서 확인했다.

> ⚠️ **`COMPOSE_PROJECT_NAME` 없이 `docker compose down -v`를 실행하면 평소 개발 DB가 지워진다.**

그 뒤 **첫 스캔(기동 뒤 300초)과 AI 분석 완료를 확인**한다 — `GET /api/v1/incidents`에서 idle-dev 카드가 **승인 대기**(`AWAITING_APPROVAL`)인지 본다. 확인은 **`api` 기동 뒤 약 6분부터** 한다. 첫 스캔 300초 · 스캔 약 8초 · 분석 주기 대기 최대 3초 · 카드 3장 분석 약 29초(§결정 기록 ④)를 더한 **추정**이다(앱 타이머로 잰 값이 아니다). **시간이 지났다고 넘어가지 않고, 3장이 모두 승인 대기인지로 판정한다.**

> ⚠️ **키 문제는 앱이 따로 알려 주지 않는다** — 화면의 신호는 카드 배지뿐이고, 로그에도 거절 사유가 남지 않는다. 위 키 확인 줄이 `OK`를 찍지 않으면 카드가 생기기 전에 잡힌다.
...
같은 스캔이 **카드 3건**을 만든다(idle-dev · 미사용 SG `vigilantis-seed-unused` · 미연결 EBS). 나머지 둘은 공통 시드 구성으로 유지한다 — LocalStack 사전 검증 테스트(`apps/core-api/services/tests/test_precheck_localstack.py`)도 같은 시드 구성을 쓴다. 시연 프로젝트 `vigilantis-demo`의 LocalStack은 개발 프로젝트와 별개이므로 시연 쪽 삭제가 개발 테스트 자원을 지우지는 않는다. **대본에서 한 줄로 설명한다**: "스캔이 서버 1대 말고도 미사용 자원 2건을 함께 찾았다." **두 카드는 시연 중 누르지 않는다**(결정 ⑦) — INC-004 목록의 「승인 대기」 프리셋에서는 `조치 실행`, INC-002 상세에서는 `승인하고 삭제`로 보인다(PR #388). EBS 카드는 이름 없이 리소스 ID(`vol-` 접두)로 보인다(수집이 EBS 이름을 비워 둔다). 모델 호출은 사전 준비 1회에 **카드 3건**이고, 모두 한 번에 성공하면 **7회**다 — idle-dev 3회(근거 요약 · 추천 · 절감 단가), 미사용 SG · 미연결 EBS 각 2회(근거 요약 · 추천). 일시 오류(제한시간 · 429 · 5xx 등)가 나면 호출 1건을 최대 3번까지 보내므로(`OPENAI_MAX_ATTEMPTS`) 늘어날 수 있다. 근거 요약이 실패하면 추천을 건너뛰어 줄어든다(PR #376 리뷰 재현: 잘못된 키 3회).
...
| 9 | 원복 완료 | ✅ 실경로 · **무대 끝** · 확인 화면을 바꾼다 | **AST-001의 인스턴스 유형은 수집만 갱신한다**(실행 경로는 자산 정보를 바꾸지 않는다) — 축소 전과 원복 뒤가 같은 값으로 보여 복귀를 증명하지 못한다. **실행 상태 패널의 "이전 상태로 복구했습니다."(원본 `ROLLED_BACK` — `apps/web/src/components/incidents/execution-status-panel.tsx`)로 확인**한다. 화면은 실행 단계(정지 → 유형 변경 → 기동)를 표시하지 않는다(API 계약에 단계 목록이 없다). **⚠️ 그 패널은 이 화면을 떠나면 사라진다** — `incident-detail.tsx`가 **방금 누른 실행의 로컬 상태**(`outcome`)나 `?execution=<실행 ID>` 딥링크(#179)로만 그린다. 5번에서 상세로 자동 이동한 뒤 **9번까지 이 화면을 떠나지 않는다.** 떠났다면 **「수행된 조치」의 `복구 완료` 배지**(원본 실행 · 서버 값이라 새로고침해도 남는다)로 확인한다 — 2026-09-23 브라우저 실측에서 확인(§결정 ⑩) | Incident는 **종료 판단 대기**로 남긴다 — **[종료 판단]은 무대에서 누르지 않는다**(§반복). 누르면 다음 스캔이 같은 서버에 새 카드를 만들고 모델까지 불러 T2 도중에 카드가 뜰 수 있다 |

### T2 · SecOps

**주입 전 전제(#380)**: 사전 준비 스캔이 끝나 `vigilantis-seed-idle`이 수집 회차와 함께 DB에 존재하고 소멸 상태가 아닌지 확인한다. 접수 시 대상 EC2와 직접 SG/NACL 관계 사본을 근거로 보존하며, 대상 근거를 확보하지 못하면 최신 자료로 대체하지 않고 분석이 실패한다. 이 경우 주입을 진행하지 않고 수집 상태부터 확인한다. **NACL 관계는 대상 EC2와 같은 수집 회차여야 차단 후보의 근거로 남는다.** 시연의 `--prepare-inbox` 주입은 관측만 기록하므로 **모의 SSH 로그 발췌는 붙지 않는다**. 이 경로에서 AI에 전달되는 자산 근거는 대상 EC2와 직접 SG/NACL 관계 사본이다.
...
| 7 | 원클릭 해제 | ✅ 실경로 · 해제 후보는 차단이 닫힌 주기에 선다(#329) | **같은 INC-002 상세 [승인하고 해제] → ACT-001 [실행]**(둘째 승인 · 실행 클릭 2회, T2 합계 4회) — 해제 후보에는 `차단 안 함`이 붙지 않는다(#363 · PR #388) | 핵심 컷 |
...
| 항목 | 10/1 판정 | 막히면 |
| --- | --- | --- |
| WS 실시간 갱신(§대조 6번) | 🔶 **실배달 확인은 9/29(화) 리허설 사전 준비**로 확정(김세혁 9/22 리뷰 승인). 코드 경로는 `apps/web/src/components/realtime-provider.tsx` → `/api/v1/ws`. 카드가 새로고침 없이 승인 대기로 바뀌는지 확인·기록은 김승철, FE 수정 담당은 김세혁 **· 2026-09-22 결정 ⑨ 자동 실측에서 서버 이벤트 23건 직접 수신(연결 표식 1건은 별도) 확인 — 화면 확인은 아니어서 9/29 정식 확인은 유지한다.** **· 2026-09-23 브라우저 실측(§결정 ⑩)에서 T2 카드가 새로고침 없이 분석 중 → 승인 대기로 바뀌는 것을 화면에서 확인했다. 다만 목록 상단 프리셋 칩의 건수는 함께 바뀌지 않는다(아래 행).** | **리허설에서 실배달이 실패하면 FE를 고치지 않고 새로고침 컷으로 확정한다.** 대본의 동작 하나가 늘 뿐 흐름은 같다 |
| 목록 프리셋 칩의 건수가 실시간으로 바뀌지 않는다 | 🔶 **2026-09-23 브라우저 실측에서 발견**(§결정 ⑩). 카드 배지는 WS로 바뀌는데 위쪽 「승인 대기 N」 칩은 **서버 렌더 시점 값**이라 그대로다 — `incident-list-page.tsx`가 `pendingCount`(그 화면이 받은 목록에서 센 건수)를 prop으로 내려보내고 `incidents-view.tsx`가 그대로 그린다. **T2 무대에서 이 순간이 보인다** — 카드는 「승인 대기」인데 탭은 `0`이다. 고칠지 대본으로 덮을지는 **9/28(월) 컷에서 판단**(FE · 김세혁) | 고치지 않으면 **탭을 가리키지 않고 카드만 짚는다.** 흐름은 같다 |
...
| ⑥ | **공격 경로 표시(#362)** — 완성되면 T2-1·T2-8 대본을 되돌릴지 | **판단만 남음** — 계약(DoD ①·②)은 PR #374로 9/17(목) 머지됐고, **화면(DoD ③)도 PR #398로 9/22(화) 머지됐다.** 2026-09-23 브라우저 실측에서 DSH-001 자산 토폴로지에 **외부 출발지 열의 「관측 출발지 `203.0.113.10`」 노드와 공격 경로 화살표**가 실 데이터로 뜨는 것을 확인했다(§결정 ⑩). **되돌릴지는 그대로 9/28(월) 릴리스 컷(#349)에서 판단**하며, 판단 전까지 대본은 지금 문구(T2-1 "붉은 노드" · T2-8 "노드 정상 복귀")를 유지한다 |
| ⑦ | 미사용 SG · 미연결 EBS 카드 — 시연 중 누르는가 | **누르지 않는다 — 김세혁 2026-09-22 리뷰에서 처분·근거 정정 승인(PR #391 최종 리뷰).** 두 자원은 **시연 트랙(T1·T2)에 쓰이지 않는다.** 따라서 실행 구현이 추가돼도 이 처분은 유지한다. 코드 대조 기준 `dev 7969bb7`에서 SG 삭제 런북은 `_RUNNERS`에 없어 미지원 예약이 비종료로 남는다(#368). EBS 삭제는 구현됐지만(PR #390), 삭제 후 시드를 재실행하면 이름표로 찾아 없어진 볼륨을 **새 ID로 생성**한다(`scripts/seed_localstack.py`의 EBS 생성부). 시연 LocalStack은 개발 것과 별개이고, 세션 사이 시드 재실행도 예정돼 있어 개발 테스트 대상 소실이나 준비 전체 재시작이 근거는 아니다. 실제 위험은 새 ID가 새 자산·EBS Incident로 이어져 기존 카드와 함께 남으며 **카드 3건이라는 대본이 어긋날 수 있음**이다. INC-004 목록의 버튼은 `조치 실행`, INC-002 상세에서는 `승인하고 삭제`다. 잘못 눌렀다면 ACT-001의 **「⚠ 파괴적 조치입니다」 경고를 보고 [취소]**한다 |
| ⑧ | 9/18 이후 머지분 대조 | **2026-09-22 · 김승철 — 코드 대조 완료 · 반영 문구 승인(PR #391 최종 리뷰).** **#376 머지 `5235a41` 이후 `dev 7969bb7`까지 전량**(#380·#378·#383·#385·#384·#386·#388·#387·#390·#389·#392·#393)을 대조했다. 영향과 제외 이유는 아래 표. 코드 확인과 화면 실측은 구분하며 **실경로 재확인은 9/29(화) 리허설**에서 한다 |
| ⑨ | LocalStack T1·T2 실경로 사전 실측(v4) | **2026-09-22 · 자동 스크립트 판정 PASS.** 측정 기준 `origin/dev 7969bb7`. T1 원본 `ROLLED_BACK` · T2 차단·해제 각각 `SUCCESS`, NACL 규칙 0→1→0. 서버 WS 이벤트 23건 직접 수신. **2회차는 카드 준비까지만 확인했으며 브라우저 화면은 미확인이다** — 그 빈 칸은 2026-09-23 브라우저 실측(아래 ⑩)이 채웠다. 시간 기준·조건은 아래 실측표. **결정 ④와 9/29(화) 정식 리허설의 사람 조작·화면 확인은 유지한다.** |
| ⑩ | **브라우저 화면 관통 실측(사람 조작)** | **2026-09-23 · 김세혁 — T1·T2 전 구간을 브라우저에서 클릭으로 관통.** 측정 기준 `origin/dev e4a2959`. T1 원본 `ROLLED_BACK` · T2 차단·해제 각각 `SUCCESS` · NACL 커스텀 규칙 0건 · 두 Incident 모두 `AWAITING_CLOSURE`. **결정 ⑦대로 미사용 SG·미연결 EBS 카드는 누르지 않았다.** 조건·발견은 아래 §결정 ⑩. **9/29(화) 리허설의 정식 확인(소요 시간 재측정 R10 · 실행 디스패치 3회 연속 · WS 정식 확인)은 유지한다** |

**결정 ⑧ 대조 범위** — 아래의 영향 없음은 이 컷 시트의 T1/T2 동선·입력에 한정한다.

| PR | 컷 시트 영향 · 반영 |
| --- | --- |
| #380 | 접수 시 대상 EC2·직접 SG/NACL 근거 보존. 대상 근거가 없으면 분석 실패, 최신 자료로 대체하지 않음 → T2 주입 전 DB 수집 상태 확인 추가 |
| #378 | IPv6 표기 정규화. T2 입력 S3는 IPv4 `203.0.113.10/32`이므로 동선·입력 변경 없음 |
| #383 | 관계 분류를 회차 상태 대신 실패 라벨 기준으로 정정. 대상 ARN·버튼·시드 입력 변경 없음 |
| #385 · #384 | 각각 모의 로그 익명화 검사·README 정리. 기존 S3 골든 주입 명령과 컷 동선 변경 없음 |
| #386 | 분석 결과와 종료 조건 정리. FE 변경 없음. 세션 사이 [종료 판단]은 비종료 실행이 없고 SecOps 분석이 종료된 상태에서 수행(`workflows.resolve_incident`) |
| #388 | 동작별 버튼 문구는 DSH-001 제안 카드·INC-002 상세에만 적용. T1은 INC-004 「승인 대기」 → `조치 실행` → 모달 `실행`, T2는 상세 → 차단/해제 버튼 → 모달 `실행`으로 반영 |
| #387 · 시드 #294 | 자산 관제 대시보드 추가. INC-004 `/asset-incidents` · INC-001 `/incidents` · AST-001 `/assets` · INC-002 상세 경로 유지. 네트워크 메트릭 시간당 4표본·분할 적재와 고아 NACL 정리 추가, idle-dev CPU 프로필 유지. 절감 예상은 AST-001에 있어 T1 대본 편입은 9/28 컷에서 판단 |
| #390 | EBS 삭제 구현 → 결정 ⑦에 구현 상태와 새 ID 재생성 위험 반영 |
| #389 | SecOps 입력·프롬프트 변경 → T2-6 판단 근거 문구를 9/29 리허설에서 재확인 |
| #392 | 정확한 `vigilantis:role=isolation` 태그의 SG를 미사용 판정에서 제외. 시드 `vigilantis-seed-unused`에는 그 태그가 없어 기존 카드 전제 유지. 전체개방 SG는 태그보다 위협 판정이 우선이라 T2 옆 SG의 빨강 전제도 유지 |
| #393 | 자산 상세의 객체·태그 표시 수정. AST-001 상세 표시는 개선되며 승인 버튼·T1/T2 동선 변경 없음 |
...
2026-09-22 10:49–11:01(KST), `origin/dev 7969bb7`에서 별도 `vigilantis-demo` 스택으로 측정했다. LocalStack 시드 → 실제 수집·판정·모델 분석 → HTTP 승인 → 디스패처 실행·판정 경로를 사용했다. 실행 주기 5초, AI 분석 주기 3초, Status Check 대기 2초·최대 3회, 스캔 주기 300초다. **브라우저 클릭 대신 HTTP를 호출하고 WS 클라이언트로 이벤트를 받았다.** 아래 시간은 1회 사전 실측값이며 리허설의 소요 시간 보장값이 아니다.

| 구간 | 실측 결과 · 시간 기준 |
| --- | --- |
| 사전 준비 | `GET /api/v1/assets`의 첫 HTTP 200 응답 확인부터 FinOps 카드 3장 모두 `AWAITING_APPROVAL`까지 **342.5초**. 키 사전 확인 등을 제외한 스크립트의 준비 단계는 337.0초다. 컨테이너 시작부터 잰 시간이 아니다 |
| T1 | m5.2xlarge → m5.large 기동 직후 헬퍼가 `stop_instances` 주입. 승인 요청부터 원본 `ROLLED_BACK` 확인까지 **17.3초**. 헬퍼 준비를 포함한 단계 전체는 21.0초. Incident는 `AWAITING_CLOSURE` |
| T2 | 위협 주입 명령 시작부터 해제 뒤 `AWAITING_CLOSURE` 확인까지 **23.1초**. 차단·해제 각각 `SUCCESS` 1건, NACL 커스텀 규칙 **0→1→0**. NACL 전후 조회를 포함한 단계 전체는 26.4초 |
| 2회차 준비 | T1·T2 종료 판단 HTTP 200·`RESOLVED`. SecOps는 **재주입 명령 완료 뒤 11.3초**, 새 idle-dev FinOps는 **반복 단계 시작부터 225.3초**에 승인 대기 확인. NACL 조회를 포함한 전체 단계 230.9초. 최종 FinOps 3장·SecOps 1장 모두 승인 대기, NACL 커스텀 규칙 0건. **시드 재실행과 2회차 T1·T2 실행 완료는 이번 측정에 포함하지 않았다** |
| WS | 서버 이벤트 **23건** = `INCIDENT_CREATED` 6 + `INCIDENT_UPDATED` 13 + `EXECUTION_UPDATED` 4. 기록 24개 중 1개는 클라이언트의 연결 표식이며 서버 이벤트가 아니다. 수집 기록상 끊김 표식 0건. **실제 화면 자동 갱신 확인은 9/29에 한다** |

**WS와 상태 조회를 구분한다.** T1 승인 접수·실행 시작에는 별도 이벤트를 받지 않았고, 자동 원복 시작의 원본 `ROLLBACK_INITIATED`와 원복 완료의 **자식 실행 `SUCCESS`**를 받았다. 원본 `ROLLED_BACK`은 HTTP 조회로 확인했다. `INCIDENT_CREATED`·`INCIDENT_UPDATED`의 데이터는 `incident_id`이며, 상태값 자체를 싣지 않는다. 인시던트 이벤트를 받으면 화면을 재조회한다. 실행 이벤트는 해당 실행을 구독한 화면에 상태를 전달하고 최종 상태에서는 재조회도 한다(`realtime-provider.tsx`). 소켓 수신만으로 화면 갱신이나 모든 중간 단계의 표시까지 확인했다고 보지 않는다.

### 결정 ⑩ 브라우저 관통 실측 조건과 확인 범위

2026-09-23(수) 10:36–10:48(KST), `origin/dev e4a2959`에서 별도 `vigilantis-demo` 스택으로 돌렸다. **결정 ⑨와 다른 점은 하나다 — 사람이 브라우저에서 클릭했다.** 승인·실행·해제를 전부 화면 버튼과 ACT-001 모달로 눌렀고, 상태는 화면에서 읽었다(수치 확인만 HTTP·AWS 조회를 병행했다).

**주기 값이 §시연 환경변수와 다르다.** 실행 주기 10초·AI 분석 주기 30초·Status Check 대기 15초 최대 12회(모두 기본값)로 돌렸다 — 같은 작업 트리를 다른 작업이 쓰고 있어 `.env`를 고치지 않았다. **그래서 아래 시간은 시연 값(5·3·2×3)으로 잰 결정 ⑨보다 길다.** 경로 성립 판정에는 영향이 없고, **소요 시간의 기준값은 결정 ④·⑨와 9/29(화) 리허설 재측정(R10)이다.**

| 구간 | 확인한 것 |
| --- | --- |
| 사전 준비 | `api` 기동 10:36 → 첫 스캔 10:41:40 → **카드 3장 모두 승인 대기 10:42:57**(기동 +약 6.5분 — §사전 준비의 "약 6분부터 확인" 추정과 맞는다). 키 확인 줄 `OK`. 모델 호출 실패·재시도 없음 |
| T1 | 10:44:23 INC-002 상세 **[이 조치 실행] → 모달 [실행]**(클릭 2회) · m5.2xlarge → m5.large. 헬퍼가 기동 직후 **`+7.91s`** 에 `stop_instances` 주입. 10:47:26 원본 `ROLLBACK_INITIATED` + 자식 `RUNBOOK_EC2_REVERT_SIZE` 시작 → **10:47:42 원본 `ROLLED_BACK` · 자식 `SUCCESS` · Incident 종료 판단 대기**. 화면 「수행된 조치」는 **`복구 완료`**(원본) + **`완료`**(자식) 두 줄로 남는다 |
| T2 | 10:45:56 주입 → 카드 생성(**보안 · 분석 중 · 선제 차단됨 · 위험도 높음**) → **새로고침 없이 승인 대기로 전환**(화면 확인) → 10:47:30 **[승인하고 차단] → [실행]** → 10:47:36 `SUCCESS` → 해제 후보가 붙은 뒤 10:48:14 **[승인하고 해제] → [실행]** → 10:48:16 `SUCCESS`. **NACL 커스텀 규칙 0건**(AWS 조회로 확인) · Incident 종료 판단 대기 · 제안 조치 "실행할 제안이 없습니다." |
| 토폴로지 | DSH-001 자산 토폴로지에 **외부 출발지 열 · 「관측 출발지 `203.0.113.10`」 노드 · 공격 경로 화살표**가 실 데이터로 떴다(#362 / PR #398). 진입(대상 그룹)·부속 열의 **`수집 실패`** 표기는 LocalStack Community에 `elbv2`·`autoscaling`이 없어서다([ADR-0006](adr/0006-localstack-team-standard-env.md) §4) — `GET /assets` 봉투의 `uncollected`가 화면에서 "0건"과 "못 가져왔다"를 가르고 있다는 뜻이다 |

**이 실측이 바꾼 대본 2곳.**

1. **T1-9 확인 화면** — 실행 상태 패널은 **그 화면을 떠나면 사라진다**(방금 누른 실행의 로컬 상태나 `?execution=` 딥링크로만 그린다 · #179). 9번까지 상세를 떠나지 않는 것을 대본에 못 박고, 떠났을 때의 확인점으로 **「수행된 조치」의 `복구 완료` 배지**를 적었다(§T1 9번).
2. **목록 프리셋 칩** — 카드 배지는 WS로 바뀌지만 「승인 대기 N」 칩은 서버 렌더 시점 값이라 `0`인 채 남는다. **T2에서 심사위원 눈에 보이는 자리**라 §공통에 행을 더했고, 고칠지 대본으로 덮을지는 9/28(월) 컷 판단이다.
...
| `RUNBOOK_EBS_DELETE_UNATTACHED` (P1) | 입력 스키마와 실행 경로는 구현됐다(PR #390). 시드 볼륨을 보존하고 T1·T2에 집중하기 위해 제외한다(결정 ⑦의 확정안) |
...
| 5 | 화면 구현 상태 | 아래 표 | 카드별 |
| 6 | **WS 이벤트로 화면이 실시간 갱신되는 것** | FE 연동 구현됨 — 소켓 수명주기·이벤트 3종·Toast·재연결(#168 / PR #181). 로컬 `core-api`로 **연결·중단·자동 복구 확인**. **2026-09-22 서버 이벤트의 소켓 수신은 확인했다(결정 ⑨). 브라우저 화면의 자동 갱신은 미확인이다.** 막던 이유였던 "코어 DB가 비었다"는 **자산에 대해서는 풀렸다**(2026-09-02 — `scripts/load_golden_assets.py`로 골든 FinOps 전량 적재). Incident를 만드는 계층(아래 9번)도 섰다 — **남은 것은 브라우저 화면의 자동 갱신 확인**이다 | **9/29(화) 리허설 사전 준비로 확정**(김세혁 9/22 리뷰 승인). 확인·기록 김승철 / FE 수정 담당 김세혁. **실배달이 실패하면 이번 시연을 위한 FE 수정 없이 새로고침 컷으로 확정한다**(§공통과 같은 결정). **2026-09-22 결정 ⑨ 자동 실측: 소켓 직접 수신 확인(화면 확인 아님) — 9/29 정식 확인은 유지** |
...
| CMN-001 실시간 | ✅ 연동 완료(#168 / PR #181). **서버 이벤트 소켓 수신 확인(2026-09-22 결정 ⑨), 브라우저 화면 자동 갱신 미확인** — 위 대조 6번 |
