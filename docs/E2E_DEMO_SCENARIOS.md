...
| 항목 | 10/1 판정 | 막히면 |
| --- | --- | --- |
| WS 실시간 갱신(§대조 6번) | 🔶 **실배달 확인은 9/29(화) 리허설 사전 준비**로 확정(김세혁 9/22 리뷰 승인). 코드 경로는 `apps/web/src/components/realtime-provider.tsx` → `/api/v1/ws`. 카드가 새로고침 없이 승인 대기로 바뀌는지 확인·기록은 김승철, FE 수정 담당은 김세혁 **· 2026-09-22 결정 ⑨ 자동 실측에서 서버 이벤트 23건 직접 수신(연결 표식 1건은 별도) 확인 — 화면 확인은 아니어서 9/29 정식 확인은 유지한다.** **2026-09-23 결정 ⑩ 사람 조작 실측에서 새로고침 없이 T1 원복 문구 · T2 카드 생성 · 차단·해제 문구가 화면에 나타났다 — 9/29 정식 확인은 유지한다.** | **리허설에서 실배달이 실패하면 FE를 고치지 않고 새로고침 컷으로 확정한다.** 대본의 동작 하나가 늘 뿐 흐름은 같다 |
...
| ⑦ | 미사용 SG · 미연결 EBS 카드 — 시연 중 누르는가 | **누르지 않는다 — 김세혁 2026-09-22 리뷰에서 처분·근거 정정 승인(PR #391 최종 리뷰).** 두 자원은 **시연 트랙(T1·T2)에 쓰이지 않는다.** 따라서 실행 구현이 추가돼도 이 처분은 유지한다. 코드 대조 기준 `dev e4a2959`에서 **두 삭제 런북 모두 실행 경로가 있다** — 미사용 SG 삭제는 PR #399(#368)로 `_RUNNERS`에 등록됐고(원복 `RUNBOOK_SG_RECREATE`는 관제자 승인 전용), EBS 삭제는 PR #390이다. 누르면 실제로 지워지고, 삭제 후 시드를 재실행하면 이름으로 찾아 없어진 자원을 **새 ID로 생성**한다(`scripts/seed_localstack.py`의 `_ensure_sg`·EBS 생성부). 시연 LocalStack은 개발 것과 별개이고, 세션 사이 시드 재실행도 예정돼 있어 개발 테스트 대상 소실이나 준비 전체 재시작이 근거는 아니다. 실제 위험은 새 ID가 새 자산·EBS Incident로 이어져 기존 카드와 함께 남으며 **카드 3건이라는 대본이 어긋날 수 있음**이다. INC-004 목록의 버튼은 `조치 실행`, INC-002 상세에서는 `승인하고 삭제`다. 잘못 눌렀다면 ACT-001의 **「⚠ 파괴적 조치입니다」 경고를 보고 [취소]**한다 |
...
| ⑨ | LocalStack T1·T2 실경로 사전 실측(v4) | **2026-09-22 · 자동 스크립트 판정 PASS.** 측정 기준 `origin/dev 7969bb7`. T1 원본 `ROLLED_BACK` · T2 차단·해제 각각 `SUCCESS`, NACL 규칙 0→1→0. 서버 WS 이벤트 23건 직접 수신. **2회차는 카드 준비까지만 확인했으며 브라우저 화면은 미확인이다.** 시간 기준·조건은 아래 실측표. **결정 ④와 9/29(화) 정식 리허설의 사람 조작·화면 확인은 유지한다.** |
| ⑩ | T1+T2 무대 소요 시간 사전 실측(R10 — 시연 시간 위험 번호, SSOT §현재 위치와 같은 표기 · 목표 ≤ 8분) | **2026-09-23 · 김승철 — 합계 6분 54.5초, 8분 안.** 측정 기준 `origin/dev e4a2959` · 별도 `vigilantis-demo` 스택 · 웹 운영 빌드(`next start`). 사람이 브라우저로 **T1-5 ①(스캔 시점 확인)부터 T2-8(해제 완료 확인)까지** 조작하고 단계마다 스톱워치로 끊었다. T1 3분 3.1초 · 전환 12.7초 · T2 3분 38.7초. T1 원본 "이전 상태로 복구했습니다." · T2 차단·해제 각각 "조치가 완료됐습니다." 확인, **새로고침 0회**. **조작자(김승철)가 설명 시간을 어림해 넣은 값이며 발표자 본인의 설명은 아니다** — 설명 포함 확정값은 9/29(화) 리허설에서 발표자·조작자 조합으로 다시 잰다. 단계별 값은 아래 실측표 |
...
2026-09-23 09:51–09:58(KST) 1회. 단계 구분은 이 컷 시트의 T1·T2 표를 따랐고, **단계를 실제로 마쳤을 때(결과를 화면에서 확인한 순간) 끊었다.** 미리 정해 둔 단계별 목표 시간은 보지 않고 쟀다. 같은 날 앞선 1회는 단계를 마친 순간이 아니라 그 목표 시간에 맞춰 끊어 실제 소요 시간이 아니므로 버렸다.

| 트랙 | 단계 | 실측 | 메모 |
| --- | --- | ---: | --- |
| T1 | 5 ① 스캔 시점 확인 | 17.8초 | |
| T1 | 5 ② 실패 주입 헬퍼 띄우기 | 18.5초 | `대기 중` 확인까지 |
| T1 | 5 ③ 관제자 승인 | 57.1초 | INC-004 「승인 대기」 프리셋 → `조치 실행` → 모달 `실행` |
| T1 | 6–8 실행 → 실패 → 자동 원복 | 60.9초 | 결정 ⑨의 17.3초는 전날 HTTP 승인 요청부터 원본 `ROLLED_BACK` 확인까지 잰 값이다. 이번 사람 조작 실측과 회차·측정 구간이 달라, 차이를 화면 확인·설명 시간으로 단정하지 않는다 |
| T1 | 9 원복 완료 확인 | 28.8초 | |
| 전환 | T1 → T2 | 12.7초 | |
| T2 | 1 위협 주입 | 25.4초 | |
| T2 | 2–4 HIGH · 선제 차단됨 · 가드레일 | 61.9초 | 카드 생성·분석 대기 포함 |
| T2 | 5 승인 → 차단 | 20.7초 | |
| T2 | 6 판단 근거 읽기 | 60.2초 | 가장 긴 설명 구간 |
| T2 | 7 원클릭 해제 | 20.5초 | 모달 버튼은 `실행` — 해제 후보도 `recommendations` 경로(`variant: ACTION`)라 [해제 실행]이 아니다(§T2 표 아래 설명) |
| T2 | 8 해제 완료 확인 | 30.0초 | |
| **합계** | | **6분 54.5초** | 8분 대비 여유 65.5초 |

**설명을 붙이면 여유가 가장 먼저 줄어드는 곳은 T2-2–4와 T2-6이다**(합 2분 2.1초). T2-2–4는 AI 분석을 기다리는 시간이라 줄이기보다 가드레일 설명을 그 안에 넣고, T2-6은 판단 근거 중 한 줄만 짚는 대본으로 줄일 수 있다. 확정은 9/29(화) 리허설에서 한다.
