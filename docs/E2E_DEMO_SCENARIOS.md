...
| 항목 | 10/1 판정 | 막히면 |
| --- | --- | --- |
| WS 실시간 갱신(§대조 6번) | 🔶 **실배달 확인은 9/29(화) 리허설 사전 준비**로 확정(김세혁 9/22 리뷰 승인). 코드 경로는 `apps/web/src/components/realtime-provider.tsx` → `/api/v1/ws`. 카드가 새로고침 없이 승인 대기로 바뀌는지 확인·기록은 김승철, FE 수정 담당은 김세혁 **· 2026-09-22 결정 ⑨ 자동 실측에서 서버 이벤트 23건 직접 수신(연결 표식 1건은 별도) 확인 — 화면 확인은 아니어서 9/29 정식 확인은 유지한다.** | **리허설에서 실배달이 실패하면 FE를 고치지 않고 새로고침 컷으로 확정한다.** 대본의 동작 하나가 늘 뿐 흐름은 같다 |
