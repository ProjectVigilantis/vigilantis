...
| 항목 | 10/1 판정 | 막히면 |
| --- | --- | --- |
| WS 실시간 갱신(§대조 6번) | 🔶 **코드 경로는 있다**(`apps/web/src/components/realtime-provider.tsx` → `/api/v1/ws`) · **실배달 확인을 리허설 9/29(화) 사전 준비로 옮기는 안은 김세혁 리뷰 대기**다(§대조 6번의 기존 9/23 기한 변경 제안). 확인 대상은 카드가 새로고침 없이 승인 대기로 바뀌는지다. 확인·기록 김승철, 고칠 일이 생기면 김세혁(FE) | 화면 새로고침 — 대본의 동작 하나가 늘 뿐 흐름은 같다 |
