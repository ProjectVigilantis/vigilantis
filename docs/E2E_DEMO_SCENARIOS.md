...
> **담당**: 박지현 (QA & Scenario) · **이슈**: #132 · **작성**: 2026-08-25 · **현황 갱신**: 2026-08-31 (김세혁 — §대조 필요 목록 1·2번 상태·원천 재지정 / 박지현 — 본문 🔶 잔여 정리·번호 표기) · 2026-09-01 (박지현 — §대조 1번 ② 해소 반영) · 2026-09-02 (박지현 — T1 1단계 골든 실경로 실측·기재, §대조 8·9번 신설) · 2026-09-04 (박지현 — 골든 EBS 편입 반영, §자산 화면의 분포 숫자를 재현 명령으로 대체) · 2026-09-08 (김세혁 — §T1 7·8·9번 상태 축 정정, §대조 3번 분리·9번 재서술)
...
남은 🔶 는 **넷**이다 — 그 배선(1번) · Status Check **실패 주입 방법**(3번 — 3번의 자동 원복 엔진 쪽은 2026-09-03에 해소됐다, #241 / PR #256) · **골든 자산 유형 3/7종으로 토폴로지 뷰가 mock인 것(8번)** · **판정→Intake 배선 부재로 T1 2단계 이후와 T2의 Incident 생성이 막힌 것(9번)**. 8·9번은 2026-09-02에 신설했다. **막혀 있던 것이 늘어난 게 아니라, mock으로 덮여 안 보이던 것을 목록에 올린 것이다** — 같은 날 자산 화면(T1 1단계)은 골든 실데이터로 서는 것을 실측해 mock을 벗겼다.
...
**골든이 채우는 분기** — 앞 두 줄은 `apps/core-api/tests/test_golden_assets_api.py`가 **CI에서 강제한다.** 사람이 옮겨 적는 값이 아니다. 셋째 줄(자산 유형)만 사람이 지키는데, **그 줄이 낡으면 §대조 8번도 함께 낡아** 목록에서 드러난다 — 유형이 늘면 8번의 여는 조건을 고쳐야 하기 때문이다.
