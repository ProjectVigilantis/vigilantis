...
| ⑦ | 미사용 SG · 미연결 EBS 카드 — 시연 중 누르는가 | **누르지 않는다 — 김세혁 2026-09-22 리뷰에서 처분·근거 정정 승인(PR #391 최종 리뷰).** 두 자원은 **시연 트랙(T1·T2)에 쓰이지 않는다.** 따라서 실행 구현이 추가돼도 이 처분은 유지한다. 코드 대조 기준 `dev 938f296`에서 **두 삭제 런북 모두 실행 경로가 있다** — 미사용 SG 삭제는 PR #399(#368)로 `_RUNNERS`에 등록됐고(원복 `RUNBOOK_SG_RECREATE`는 관제자 승인 전용), EBS 삭제는 PR #390이다. 누르면 실제로 지워지고, 삭제 후 시드를 재실행하면 이름으로 찾아 없어진 자원을 **새 ID로 생성**한다(`scripts/seed_localstack.py`의 `_ensure_sg`·EBS 생성부). 시연 LocalStack은 개발 것과 별개이고, 세션 사이 시드 재실행도 예정돼 있어 개발 테스트 대상 소실이나 준비 전체 재시작이 근거는 아니다. 실제 위험은 새 ID가 새 자산·EBS Incident로 이어져 기존 카드와 함께 남으며 **카드 3건이라는 대본이 어긋날 수 있음**이다. INC-004 목록의 버튼은 `조치 실행`, INC-002 상세에서는 `승인하고 삭제`다. 잘못 눌렀다면 ACT-001의 **「⚠ 파괴적 조치입니다」 경고를 보고 [취소]**한다 |
...
| ⑩ | T1+T2 무대 소요 시간 사전 실측(R10 · 목표 ≤ 8분) | **2026-09-23 · 김승철 — 합계 6분 54.5초, 8분 안.** 측정 기준 `origin/dev 938f296` · 별도 `vigilantis-demo` 스택 · 웹 운영 빌드(`next start`). 사람이 브라우저로 **T1-5 ①(스캔 시점 확인)부터 T2-8(해제 완료 확인)까지** 조작하고 단계마다 스톱워치로 끊었다. T1 3분 3.0초 · 전환 12.7초 · T2 3분 38.7초. T1 원본 "이전 상태로 복구했습니다." · T2 차단·해제 각각 "조치가 완료됐습니다." 확인, **새로고침 0회**. **조작자(김승철)가 설명 시간을 어림해 넣은 값이며 발표자 본인의 설명은 아니다** — 설명 포함 확정값은 9/29(화) 리허설에서 발표자·조작자 조합으로 다시 잰다. 단계별 값은 아래 실측표 |
...
2026-09-23 09:51–09:58(KST) 1회. 단계 구분은 이 컷 시트의 T1·T2 표를 따랐고, **단계를 실제로 마쳤을 때(결과를 화면에서 확인한 순간) 끊었다.** 스톱워치는 재는 동안 예산을 가렸다. 같은 날 앞선 1회는 예산표 시각에 맞춰 끊어 실측이 아니므로 버렸다.
...
| T2 | 7 원클릭 해제 | 20.5초 | 모달 버튼은 `해제 실행` |
| T2 | 8 해제 완료 확인 | 30.0초 | |
