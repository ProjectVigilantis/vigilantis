...
> **갱신은 매주 월요일 아침 PM(김세혁)이 한다.** 주중 갱신은 마일스톤에 혼동을 줄 변경·큰 변경에 한한 예외다(아래 §현재 위치).
>
> **최종 갱신**: 2026-08-31 (김세혁)
...
> **갱신 주기**: 이 문서는 **매주 월요일 아침 PM(김세혁)이 갱신한다.** 주중에는 마일스톤에 혼동을 줄 변경이나 범위·API 계약·역할·확정 결정이 걸린 큰 변경이 있을 때만 예외로 갱신하며, 그 밖의 상황에서는 수정하지 않는다. 일반 기능·버그 수정·리팩터링·테스트 PR은 이 문서를 갱신하지 않는다.
...
- **마일스톤**: **4주차(2026-08-31 – 09-06) — P0 전반부.** 목표 = AI 추천 경로 동작 · 백업/감시 모듈 완성 · **Golden Dataset 20건 정답 100% 완결**. `RIGHTSIZING` 실행 + `get_waiter` 감시가 붙으면 5주차 자동 원복이 열린다.
- **완료 (구간 요약)**
...
- **다음 단계 (담당별 1줄 — 4주차 카드)**
  - **김세혁**: `RUNBOOK_EC2_RIGHTSIZING` 실행 경로(P0 — 백업 → `stop` → `modify_instance_attribute` → `start`, 단계별 기록, LocalStack 통합 테스트) → `get_waiter` Status Check(2/2) 감시·타임아웃 3분기(P0 — 성공 판정 경계는 인프라 부팅까지이며 앱 Health Check는 별개 축). 선행 조건(팩토리·백업 모듈·`precheck()`·typed 파라미터)은 전부 확보됐다.
  - **안성일**: 가드레일 ③ ARN Match(#177 — 4단계 중 유일한 공백, `test_guardrails.py` skip 1건과 QA 4단계 테스트가 여기 묶여 있다) → LangGraph FinOps 그래프(P0) → CoT 3줄 요약·Runbook 추천 Structured Output(P0 — `ai_recommendable` 7종만 추천). 김승철 Risk Evaluator 출력 계약 검토·승인은 산출물 대기.
  - **김승철**: **Risk Evaluator 확정·구현**(🔴 P0 — `RiskReasonCode` 값 목록·High/Medium/Low 판정 규칙·`response_mode` 매핑, 안성일 승인 필수). **§미해결 6번의 유일한 블로커이며 2026-08-31 기준 이슈 미발행·미착수다.** 이후 CloudWatch 메트릭 수집 정밀화(P1 — `SCAN_INTERVAL_SECONDS` 300초 vs 운영 전제 30분 불일치 해소 포함), 수집 실패 대응(P1 — 자산 단위 실패 사유·부분 리트라이).
  - **박지현**: 실행 계열 테스트 하네스·픽스처(#136 — 3주차 이월분, 4주차 테스트 카드가 전부 여기 붙는다) → 가드레일 4단계 통과·거절 유닛 테스트(①② 완료, ③은 #177 구현 후 skip 해제) → SecOps expected 정답 10건(김승철 Risk Evaluator 대기).
  - **유건희**: AI CoT 3줄 요약 카드 UI(P0) · AST-001 토폴로지 뷰(#146 — `RelationType` 6종 엣지·3계층 배치) · v1.6 화면 개편(#196 — 인시던트 보안/자산 2분할·GNB 4탭·종료 판단 모달). 4·5주차 FE 카드는 3주차에 대부분 선행 완료됐다.
- **주차 종료 판정 기준 (4주차) — 이 문서가 유일한 기재 위치다**

  주차 종료 판정 기준은 `docs/PROJECT_STATUS.md` 밖 어디에도 두지 않는다. 매주 월요일 PM이 AI의 추천을 받아 직접 갱신한다.
...
- **4주차 리스크**
  1. **Risk Evaluator 미착수(발현 중)** — 김승철 → 안성일 승인 → 박지현 SecOps 정답의 3단 직렬이라, 주 중반까지 착수되지 않으면 판정 기준 ⓔ의 4주차 내 충족이 불가능해진다. 미착수가 이어지면 2026-08-19(#67)·2026-08-24(#95) 선례대로 PM 대행 결정으로 전환한다.
  2. LocalStack이 `modify_instance_attribute` 일부 동작을 미지원할 수 있다 — ⓐ 착수 첫날에 확인하고, 미지원이면 미지원 API 목록화를 조기 발동한다.
