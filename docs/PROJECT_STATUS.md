...
- **다음 단계 (담당별 1줄 — 4주차 카드)**
  ...
  - **김승철**: ✅ Risk Evaluator 확정·구현(#210 / PR #206 — 리스크 1번 해소) → ✅ 수집 실패 대응(#221 / PR #222 — 자산 단위 실패 사유·리전 격리·부분 리트라이) → ✅ CloudWatch 수집 정밀화(#255 / PR #257 — 메트릭 재조회 제거·`SCAN_INTERVAL_SECONDS` config 이관. **30분 주기 전환은 미채택** — 인벤토리 describe와 메트릭 조회가 한 잡이라 주기를 늦추면 SecOps 탐지도 함께 느려진다). **남은 것**: `collection_status` 리전 스코프(#261 — 2026-09-03 안성일 확정, 구현 착수), 스캔 스케줄러 앱 배선 + 중복 실행 방지(#277 — `run_pipeline`을 부르는 코드가 아직 없다), 위협 정형화 프로덕션화(#268 / PR #269), EBS 전이·비정상 상태 판정 정책(#276), **FE 유사 서비스 비교(9/02 지시, 유건희와 협력) → 유사 서비스 제작은 5주차(2026-09-03 D1)**, FE 작업 합류(A2 — **확정 아님·검토 단계**, 9/04까지 정한다).
  - **박지현**: ✅ 실행 계열 테스트 하네스·픽스처(#136 / PR #191) → ✅ 가드레일 4단계 통과·거절 유닛 테스트(skip 0건) → ✅ SecOps expected 정답 12건(PR #223 10건 + PR #242 SSH MEDIUM 밴드 2건). **2026-09-03 A1**: **AWS 스키마·데이터 구조 확인 + 쓸 만한 외부 골든 데이터셋 조사**(웹 확인은 사람이 직접 한다). 그 밖의 남은 것: 골든 FinOps 자산 적재 경로(#262 / PR #263 — 자산 화면이 mock 없이 골든으로 선다), EBS 미연결 볼륨 골든 재작업(#264 — PR #266은 revert(#275)됐고 #276 정책 확정이 선행), 9/11 리허설 대본(#267의 결정 4건이 입력 — 답이 없으면 안건에 적힌 기본 가정대로 쓴다).
