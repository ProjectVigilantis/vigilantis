...
- **마일스톤**: **4주차(2026-08-31 – 09-06) — P0 전반부.** 목표 = AI 추천 경로 동작 · 백업/감시 모듈 완성 · **Golden Dataset 20건 정답 100% 완결**. `RIGHTSIZING` 실행 + `get_waiter` 감시가 붙으면 5주차 자동 원복이 열린다 — 세 축 모두 dev에 머지됐다(#211 · #240 · #241, 자동 원복은 2026-09-03 PR #256). **2026-09-03 회의 조정**: 유사 서비스 제작은 4주차 범위에서 빼 **5주차(9/07–9/13)로 연기**하고, **백엔드 연동 완성은 6주차(9/14–9/20)** 로 잡는다. 근거 조회·시계열 차트·시간 슬라이드 차트는 이번 범위 밖이다(§MVP 확정 범위).
- **완료 (구간 요약)**
...
- **다음 단계 (담당별 1줄 — 4주차 카드)**
  - **김세혁**: ✅ `RUNBOOK_EC2_RIGHTSIZING` 실행 경로(#211 / PR #216 — LocalStack 4.14.0의 `modify_instance_attribute` 지원을 실측으로 확인, 리스크 2번 해소) → ✅ `get_waiter` Status Check(2/2) 판정·`AWAITING_CLOSURE` 신설(#240 / PR #244) → ✅ `RUNBOOK_EC2_REVERT_SIZE` 실행·`AUTO_ON_FAILURE` 자동 발동(#241 / PR #256, 2026-09-03 머지). **남은 것**: ADR-0008 `Accepted` 전환(PR #253 머지본은 `Proposed`), `NACL_ADD_DENY`·`NACL_RESTORE` 착수 전 선행 작업(`latest_for_target` 백업 조회의 DB 배선 — #207 코멘트), Status Check 판정 불가 계약(#249), `target_instance_type` 허용값 방침(#251 — PM 결정 필요), **#267 회의 안건 1·2번(리허설 실경로 경계·시연 데이터 출처) 답변 9/04까지**.
  - **안성일**: ✅ 가드레일 ③ ARN Match(#177 — 4단계 완성, `test_guardrails.py` skip 0건) → ✅ LangGraph FinOps 그래프(#209 / PR #226) → ✅ AI 응답 품질 계측·모델 비교(#237 / PR #248, 기본 모델 교체는 PR #252) → ✅ Detection → Incident → AI 입력 계약과 두 모듈 자리(#254 / PR #258). **남은 것**: AI 요약 프롬프트 v1(#243 — 출력 계약은 섰고 문구가 남았다), 근거로 남는 인시던트 생성 Workflow(#265), #267 회의 안건 3·4번(판정→Intake 배선 시점·금액 축 계약) 답변 9/08까지.
  - **김승철**: ✅ Risk Evaluator 확정·구현(#210 / PR #206 — 리스크 1번 해소) → ✅ 수집 실패 대응(#221 / PR #222 — 자산 단위 실패 사유·리전 격리·부분 리트라이) → ✅ CloudWatch 수집 정밀화(#255 / PR #257 — 메트릭 재조회 제거·`SCAN_INTERVAL_SECONDS` config 이관. **30분 주기 전환은 미채택** — 인벤토리 describe와 메트릭 조회가 한 잡이라 주기를 늦추면 SecOps 탐지도 함께 느려진다) → ✅ EBS 전이·비정상 상태 판정 정책(#276 / PR #284 — `available`+미부착만 `UNUSED`, `in-use`·부착은 `SKIP_ACTIVE`, 그 외 전이·비정상·미상 상태는 신규 `SKIP_UNSUPPORTED_STATE`로 판정 보류. 골든 전이 케이스는 박지현 후속). **남은 것**: `collection_status` 리전 스코프(#261 — 2026-09-03 안성일 확정, 구현 착수), 스캔 스케줄러 앱 배선 + 중복 실행 방지(#277 — `run_pipeline`을 부르는 코드가 아직 없다), 위협 정형화 프로덕션화(#268 / PR #269), **FE 유사 서비스 비교(9/02 지시, 유건희와 협력) → 유사 서비스 제작은 5주차(2026-09-03 D1)**, FE 작업 합류(A2 — **확정 아님·검토 단계**, 9/04까지 정한다).
  - **박지현**: ✅ 실행 계열 테스트 하네스·픽스처(#136 / PR #191) → ✅ 가드레일 4단계 통과·거절 유닛 테스트(skip 0건) → ✅ SecOps expected 정답 12건(PR #223 10건 + PR #242 SSH MEDIUM 밴드 2건) → ✅ EBS 미연결 볼륨 골든(#264 / PR #280 — PR #266은 revert(#275)됐지만, **확정 규칙 표에 답이 있는 E1·E2 2건**은 정책 확정을 기다리지 않고 다시 냈다. #276은 **나머지 케이스의 선행**이지 이 카드 전체의 선행이 아니었다). **2026-09-03 A1**: **AWS 스키마·데이터 구조 확인 + 쓸 만한 외부 골든 데이터셋 조사**(웹 확인은 사람이 직접 한다). 그 밖의 남은 것: 골든 FinOps 자산 적재 경로(#262 / PR #263 — 자산 화면이 mock 없이 골든으로 선다), 실행 계열 DB 시드 헬퍼 공용화(#233 / PR #260), 골든 커버리지 가드를 판정 대상 유형으로 좁히기(#271 / PR #270 — 토폴로지 자산이 들어올 자리라 유건희 토폴로지 뷰와 닿는다), SecOps OPEN_IP 분기 보강(#272 / PR #273), 9/11 리허설 대본(#267의 결정 4건이 입력 — 답이 없으면 안건에 적힌 기본 가정대로 쓴다).
  - **유건희**: ✅ AST-001 토폴로지 뷰(#146). 진행 중: AI CoT 3줄 요약 카드 UI(P0) · v1.6 화면 개편(#196 — 인시던트 보안/자산 2분할·GNB 4탭·종료 판단 모달) · 승인 화면 조치 대상 문맥(#183) · **FE mock 초안과 FE 유사 서비스 비교(9/02 지시, 김승철과 협력)**. **2026-09-03 A2**: FE 작업을 김승철과 함께 진행하는 안은 **확정 아님(검토 단계)** — 9/04(금)까지 정한다. #267 회의 안건 4번(화면에 금액 축이 필요한지) 답변 9/08까지.
- **주차 종료 판정 기준 (4주차) — 이 문서가 유일한 기재 위치다**

  주차 종료 판정 기준은 `docs/PROJECT_STATUS.md` 밖 어디에도 두지 않는다. 매주 **월요일** 갱신에서 PM이 AI의 추천을 받아 직접 갱신한다 — 목요일 갱신은 이 표를 건드리지 않는다.

  | # | 기준 | 판정 |
  | --- | --- | --- |
  | ⓐ | `RIGHTSIZING` LocalStack 실행 성공 + 백업 레코드 생성 확인 | ⏳ |
  | ⓑ | `get_waiter` 감시가 성공/실패/타임아웃 3분기 반환 | ⏳ |
  | ⓒ | AI가 Golden Dataset FinOps 10건에서 CoT 3줄 + Runbook 추천 산출 | ⏳ |
  | ⓓ | 가드레일 ①②③ 통과·거절 테스트 green | ⏳ |
  | ⓔ | **Golden Dataset 20건 정답 100% 채움 — 미해결 4번·6번 동시 종결** | 4번 ✅ 종결 / 6번 ⏳ |
  | ⓕ | 수집 부분 실패 시 **실패 대상만** 재수집 동작 확인 | ⏳ |
...
- **4주차 리스크**
  1. ~~**Risk Evaluator 미착수(발현 중)**~~ ✅ **해소(2026-08-31)** — #210 / PR #206 머지. 김승철 → 안성일 승인 → 박지현 SecOps 정답의 3단 직렬은 정답 12건(PR #223 · #242)까지 끝났다.
  2. ~~LocalStack이 `modify_instance_attribute` 일부 동작을 미지원할 수 있다~~ ✅ **해소(2026-09-01)** — PR #216 실측: LocalStack 4.14.0에서 호출이 받아들여지고 `describe_instances`의 `InstanceType`이 실제로 바뀐다. 미지원 API 목록화는 발동하지 않는다.
  3. **9/11 리허설 대본이 #267의 결정 4건에 걸려 있다(2026-09-03 기준 미답)** — 1·2번(실경로/대체 컷 경계·시연 데이터 출처)은 김세혁이 **9/04까지**, 3·4번(판정→Intake 배선 시점·금액 축 계약)은 안성일·유건희가 **9/08까지** 답해야 한다. 답이 없으면 박지현이 안건에 적힌 기본 가정(T1 1–6번만 실경로 · T2 전 구간 대체 컷 · 인시던트 화면은 mock · 금액 축 없음)으로 대본을 쓴다.
...
| **안성일** | AI / Guardrail · Architect | 아키텍처·DB 스키마, FastAPI 메인·PostgreSQL, GPT-4o + 4단계 가드레일 | `AI`(가드레일·CoT) · FastAPI 아키텍처 · DB |
| **김승철** | Data & Rule Engine | CloudWatch 수집 파이프라인, Rule Evaluator(Idle EC2·미사용 SG, Skip 사유 적재) | `DATA`(자산/메트릭 수집 · Rule Engine) |
| **박지현** | QA & Scenario / Technical Writer | Golden Dataset(약 20건), E2E 시연 시나리오, pytest, 문서화 | `DOCS` · QA 시나리오 |
...
5. `README.md` — **포트폴리오·소개용**. 프로젝트를 처음 보는 사람을 위한 문서라 서술이 상세하고 갱신 주기가 길다. 현황·결정의 기준이 아니다
