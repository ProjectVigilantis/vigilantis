...
| 2026-08-25 | **이슈 자동 CLOSE 금지 — 교차검증 후 수동 CLOSE** — 커밋 푸터·PR 본문에 `Closes`·`Fixes`·`Resolves`를 쓰지 않고 **`Refs #N`으로만 연결**한다. 이 저장소는 **기본 브랜치가 `dev`**라 그 키워드가 머지 즉시 이슈를 닫는데, 리뷰 지적의 잔여 작업이 남아 있어도 "닫힘 = 해소"로 보이게 된다(실제 사례: #124·#134가 커밋 푸터로 자동 닫힘, #134는 잔여 때문에 재오픈). 머지 후 **작성자 외 1명**의 교차검증 결과를 이슈 코멘트로 남기고 수동으로 CLOSE한다. 규약 자체는 2026-08-25에 결정됐으나 어느 문서에도 등재되지 않아 뒤늦게 기록한다 — 코드화 위치는 `CLAUDE.md` §Git 작업 흐름·§커밋 메시지 규칙과 `.github/PULL_REQUEST_TEMPLATE.md`다 | 이슈 #124·#134 코멘트(2026-08-25, 박지현) |
