...
`check`는 인증 결과, invalid/authenticating user, PAM 사용자·실행자,
최대 인증 시도 초과·PAM 인증 실패·SFTP 요청의 사용자 자리를 검사한다.
`root` 같은 시스템 계정도 `mock-user`·`user-NNN` 별칭으로 바꾼다.
이 표본은 계정의 권한 수준을 보존하지 않는다.

IP는 문서용 IPv4 3개 대역과 `2001:db8::/32`만 허용한다. IPv4를 섞어 쓴 IPv6도
주소 전체로 검사하며, `::ffff:198.51.100.10` 같은 IPv4 매핑 IPv6는 내부 IPv4가
문서용 대역이어도 거부한다. IPv6의 인터페이스 식별자(`%...`)는
`IP interface identifier is not allowed`로 대역 오류와 구분해 거부한다.
`Received disconnect from <주소> port <포트>:<종료코드>:`에서는 포트·종료 코드만
IP 후보에서 제외하며 주소는 계속 검사한다. `HH:MM:SS`(00:00:00–23:59:59)는 시각으로
구분하고, 콜론으로 구분한 6쌍의 MAC 주소는 `unredacted MAC address`로 거부한다.
그 밖의 IP 후보 파싱 실패는 `invalid IP candidate`로 거부한다.

이 검사는 알려진 사용자명·주소 위치를 보완하는 제한된 검사이며, 임의의 sshd 문장이나
자유 텍스트 전체의 익명화를 보장하지 않는다. 새 표본은 공개 전에 원문에 남은 식별자를
직접 검토하고 필요한 검사와 회귀 사례를 함께 추가한다. 익명화 검사 통과는 집계기가
지원하는 메시지 형식을 늘리지 않는다.
...
저장 근거의 `record_count`는 실패·수락·보조 행의 합을 요구한다. 현재 `prepare`는
근거를 조립하기 전에 `check`를 호출하고, `check`는 제외·중복 행이 있는 사례를 거부한다.
위협으로 투영할 수 없는 unsupported 행도 거부하므로 전달 가능한 자료에서는 두 수가 같다.
현재는 이 제한을 유지한다. 제외 행을 포함한 사례를 지원할 때에는 선택 전·후 행 수와
발췌 범위를 집계·저장 계약에서 함께 정해야 한다.
