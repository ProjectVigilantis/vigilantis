...
### 갱신 주체와 시점 — 매주 월요일, PM만

- **SSOT는 매주 월요일 아침 PM(김세혁)이 갱신한다.** 주차 전환·완료 요약·담당별 다음 단계·주차 종료 판정 기준을 한 번에 정리한다. 주차 판정 기준은 PM이 **AI의 추천을 받아 직접** 확정한다.
- **그 밖의 상황에서는 SSOT를 수정하지 않는다.** 일반 기능·버그 수정·리팩터링·테스트·문서 PR은 SSOT를 건드리지 않으며, 개별 머지 이력을 SSOT에 복제하지 않는다.
- **예외는 하나다** — 마일스톤 진행에 혼동을 줄 변경이나, 위 트리거 표의 큰 변경(범위·API 계약·역할·확정 결정)이 주중에 확정된 경우. 이때만 주중 갱신을 별도 PR로 올린다.
