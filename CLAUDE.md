...
- **정기 갱신을 앞당긴 주에도 그 주 목요일 갱신을 건너뛰지 않는다**(2026-09-16 추가). 큰 변경이 주중에 확정돼 목요일 갱신을 앞당겨 수행했더라도, **그 주 목요일 회의에서 새로 확정된 결정·액션 아이템은 그날 반영한다.** 건너뛰면 그 회의의 결정이 다음 갱신일(월요일)까지 갈 자리가 없어 나중에 "그때 무엇이 기준이었나"를 판단할 수 없다. 다만 **앞당긴 갱신에 이미 담긴 현황·담당 서술은 다시 쓰지 않는다** — 목요일에 더하는 것은 그날 새로 생긴 것뿐이다.
...
- **리뷰/머지**: 최소 1명 이상(특히 백엔드↔AI↔프론트 API 접점 담당자)의 승인과 CI 통과 후 머지한다. CI는 세 잡이다 — `test`(pytest + LocalStack·PostgreSQL service container), `web`(ESLint · `next build` 타입 체크 · `node --test`, #91 / PR #176), `ai-signature`(커밋 메시지·PR 본문의 AI 서명 검사, `.github/workflows/ai-signature.yml`).
...
## GitHub 글의 정정 — 새 글이 아니라 기존 글 수정으로 한다 (2026-09-16 팀 규칙)

**이미 올린 글이 틀렸으면 그 글을 고친다. 정정본을 새 글로 올리지 않는다.** 코멘트를 여러 개 다는 것도 같다 — 지적·요청·정정이 나뉘어 올라가면 알림이 여러 번 가고, 읽는 사람은 그것을 다 읽은 뒤 **어느 쪽이 최종인지 스스로 가려내야** 한다. 정정하려고 올린 글이 오히려 무엇이 최종 요청인지를 흐린다(실제 사례: PR #326에서 9분 간격으로 리뷰 두 건이 올라갔고, 뒤엣것이 앞엣것의 정정본이었다).

**적용 대상은 GitHub에 올리는 모든 글이다** — PR 리뷰 본문 · PR 코멘트 · 이슈 본문 · 이슈 코멘트.

| 고칠 것 | 명령 |
| --- | --- |
| PR 리뷰 본문 | `gh api -X PUT /repos/{owner}/{repo}/pulls/{n}/reviews/{review_id} -F body=@<파일>` |
| PR·이슈 코멘트 | `gh api -X PATCH /repos/{owner}/{repo}/issues/comments/{comment_id} -F body=@<파일>` |
| 이슈 본문 | `gh issue edit <번호> --body-file <파일>` |
| PR 본문 | `gh pr edit <번호> --body-file <파일>` |

- **`-F`(대문자)여야 한다.** 소문자 `-f`는 값을 문자열 그대로 보내므로 위 명령을 `-f`로 실행하면 본문이 `@파일경로`라는 **글자로 덮어써진다.** `@`로 시작하는 값을 파일 내용으로 읽는 것은 `-F`뿐이다([gh api 문서](https://cli.github.com/manual/gh_api)).
- **게시 전에 할 말을 전부 모은다.** 리뷰 하나에 지적·요청·정정을 함께 담아 **한 번에** 올린다.
- **조용히 고치지 않는다.** 이미 읽은 사람이 있으므로 수정본 **맨 위에 무엇을 왜 정정했는지 한 줄**을 남긴다.
- **한 번 더 틀리면 또 수정한다.** 수정 횟수에는 제한이 없다 — 새 글을 올릴 이유가 되지 않는다.

**예외는 하나뿐 — 리뷰 판정 자체가 뒤집힐 때.** GitHub은 제출된 리뷰의 상태(`CHANGES_REQUESTED` ↔ `APPROVED`)를 본문 수정으로 바꿀 수 없다. 이때만 새 리뷰를 **한 건** 올리며, 셋을 지킨다.

1. 새 리뷰에는 **바뀐 판정과 그 사유만** 쓴다. 이전 리뷰 내용을 다시 옮기지 않는다.
2. 직전 리뷰 본문은 **철회 배너로 수정**해 새 리뷰를 가리킨다(위 표의 `PUT`).
3. **판정이 그대로면 새 리뷰를 올리지 않는다** — 요청 일부 철회 · 근거 보강 · 실측 추가 · 오탈자는 전부 본문 수정이다.

**애초에 이 상황을 만들지 않는 것이 먼저다.** #326의 두 리뷰는 **리뷰를 쓰는 동안 저자가 푸시**해서 생겼다(`6104e69` 푸시 6분 뒤에 낡은 내용이 게시됐다). **게시 직전에 head를 다시 확인한다** — `gh pr view <번호> --json headRefOid,commits`로 체크아웃한 커밋과 같은지 보고, 다르면 **게시하지 말고 다시 읽은 뒤** 쓴다.
