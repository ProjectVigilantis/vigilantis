...
class AIModelContractError(AIModelError):
    """응답이 요구한 구조화 출력으로 파싱되지 않음. 재시도하지 않는다."""
...
# Authorization 헤더는 값 전체(스킴 포함)를 줄 끝까지 가린다 — Basic·Digest·AWS4 등
# 인증 방식을 열거하면 새 방식마다 구멍이 생기고, 라벨 규칙의 (\S+)는 스킴 한 단어만
# 가려 자격증명 본체가 남는다(예: "Basic dXNlcjpwYXNz")
_AUTHORIZATION_HEADER = re.compile(r"(?i)\b(authorization\b[\"']?\s*[=:]\s*)[^\r\n]+")
...
def build_outbound_payload(request: AIModelRequest) -> dict[str, Any]:
    ...
    return {
        ...
        "user_payload": mask_outbound(dict(request.user_payload)),
    }
