"""후보 B의 저장 수용·평가 중단 경계. 실제 가격 품질과 별도인 무료 검증이다."""
...
import httpx
import pytest
from ai.evaluation.savings import numeric_validation as runner
from ai.evaluation.savings.numeric import ProposedHourlyRates, accept_numeric_rates
...
def test_unstorable_or_inconsistent_values_rejected_without_raw_values(prepared, updates):
    _, assets, _ = prepared
    diagnostics = []
    result = accept_numeric_rates(proposal(**updates), assets["A1"], diagnostics=diagnostics)
...
def test_server_context_is_bound_without_claiming_model_echo_validation(prepared):
    _, assets, _ = prepared
    a1 = accept_numeric_rates(proposal(), assets["A1"])
    a7 = accept_numeric_rates(proposal(), assets["A7"])
...
def test_existing_price_failures_are_not_corrected_or_hidden(prepared, current, target, amount):
    ...
    estimate = accept_numeric_rates(proposal(
...
def round_files(prepared, tmp_path, monkeypatch):
    ...
    runner.write(tmp_path / "approval.json", {
        ...
        "prior_attempted_calls": 2, "candidate": "B",
...
class SyntheticClient:
    ...
    def complete(self, request, response_model):
        ...
        if self.defect == "contract":
            output = proposal(current_hourly_rate="private-rejected-value")
...
def test_call_cap_and_availability_denominator(round_files, unavailable, expected_calls, passed):
    ...
    assert result["cumulative_attempted_calls"] == expected_calls + 2
    assert result["unused_user_call_limit"] == 78 - expected_calls
    assert result["explanation_source"] == "SERVER_TEMPLATE"
...
def test_explanation_is_server_text_not_model_output(prepared):
    _, assets, _ = prepared
    output = proposal()
    assert "explanation" not in output.model_dump()
    estimate = accept_numeric_rates(output, assets["A1"])
    assert assets["A1"].region in estimate.basis.explanation
    assert "서버가 두 단가의 차이에 월 730시간을 곱했습니다" in estimate.basis.explanation
    assert "모델 지식 기반 추정값" in estimate.basis.explanation
    with pytest.raises(ValidationError):
        ProposedHourlyRates.model_validate({**output.model_dump(), "explanation": "unexpected"})


def test_total_budget_rejected_before_credentials(round_files):
    frozen, assets, _, directory = round_files
    oversized = {**frozen, "prior_attempted_calls": 21}
    with pytest.raises(ValueError, match="PRIOR_BUDGET_DRIFT"):
        runner.execute(oversized, assets, client_factory=lambda hook: pytest.fail("budget exceeded"))
    assert not (directory / "started.json").exists()


def test_prior_ledger_drift_blocks_before_credentials(round_files, monkeypatch):
    frozen, assets, _, directory = round_files
    prior = directory / "prior"
    prior.mkdir()
    runner.write(prior / "metadata.json", {})
    monkeypatch.setattr(runner, "PRIOR_ROUND", prior)
    with pytest.raises(ValueError, match="PRIOR_BUDGET_DRIFT"):
        runner.execute(frozen, assets, client_factory=lambda hook: pytest.fail("ledger drift"))
    assert not (directory / "started.json").exists()
