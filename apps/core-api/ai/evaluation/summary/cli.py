...
from ai.evaluation.common.usage import (
    UsageRecordingClient as _UsageRecordingClient,
)
...
SNAPSHOT = REPO_ROOT / "apps" / "core-api" / "ai" / "evaluation" / "summary" / "summary_prompt_snapshot.json"
# 골든 파일을 손으로 고른다 — 고정 세트는 스냅샷 fixed_set에 파일 목록으로 박혀 있어, 파일이 늘 때
# 조용히 넓어지지 않고 사람이 여기와 스냅샷에 추가한 뒤 기준선을 다시 잰다(apps/core-api/ai/evaluation/summary/baseline.md).
...
def _fixed_set_drift(fixed_set: dict[str, object]) -> list[str]:
    ...
    이전 판을 새 세트에서 다시 만들어 짝을 맞추게 한다(apps/core-api/ai/evaluation/summary/baseline.md §기준선).
    """
...
def _estimate(cases: list[EvalCase], repeats: int) -> None:
    ...
    print(f"모델 호출 최대 {len(cases) * repeats * FINOPS_MODEL_CALLS}회 (요약 1 + 후보 1; 단가는 Workflow 후속 처리)")
            ...
            json.dumps(
                {
                    ...
                    "request_sha256": finops_request_fingerprint(),
