...
# 채점·요청 구성은 ai/evaluation/summary/judge.py에 있고 여기는 파일·호출·집계만 한다.
#
...
def write_calibration_sheet(
    ...
    lines = [
        ...
        "기준은 apps/core-api/ai/evaluation/summary/summary_defects.md의 항목 본문이다.",
        "",
    ]
...
def main() -> int:
    ...
    # 다시 만든 뒤 온다(apps/core-api/ai/evaluation/summary/baseline.md §기준선). CI로는 막지 않고 여기서 거절한다
    if any(raw.get("fixed_set") is None for raw in sources):
        ...
        print(
            ...
            "이전 판을 새 세트에서 다시 만든 뒤 판정한다(apps/core-api/ai/evaluation/summary/baseline.md §기준선)",
            file=sys.stderr,
        )
        return 1
