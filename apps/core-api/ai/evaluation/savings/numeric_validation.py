"""후보 B의 고정 60회 충분성 평가. 기본 실행은 키를 읽지 않는 오프라인 검증이다."""
...
from ai.evaluation.savings.compact import (
    ProposedHourlyRates as BaselineRates,
)
from ai.evaluation.savings.compact import compact_request as baseline_request
from ai.evaluation.savings.isolation import score_price_only_report
from ai.evaluation.savings.numeric import (
    EXPLANATION_SOURCE,
    PROMPT_VERSION,
    SYSTEM_PROMPT,
    ProposedHourlyRates,
    accept_numeric_rates,
    numeric_request,
    without_explanation_instructions,
)
...
ROUND = Path(__file__).parent / "results/20260916-numeric-output-1"
PRIOR_ROUND = Path(__file__).parent / "results/20260916-output-simplification-1"
USER_TOTAL_CALL_LIMIT = 80
...
def capture_wire(asset, *, baseline=False):
    ...
    if baseline:
        output["explanation"] = "오프라인 검증용 합성 응답"
    ...
    with OpenAI(api_key="offline-only", base_url="https://unit.test/v1", max_retries=0,
        ...
        make_client(sdk).complete(
            baseline_request(asset) if baseline else numeric_request(asset),
            BaselineRates if baseline else ProposedHourlyRates,
        )
    if len(captured) != 1:
...
def current_preflight():
    prior = read(PRIOR_ROUND / "metadata.json")
    prior_analysis = read(PRIOR_ROUND / "analysis.json")
    prior_calls = len(prior["calls"])
    if (prior["status"] == "RUNNING" or prior_calls + MAX_CALLS > USER_TOTAL_CALL_LIMIT
            or prior_analysis["metadata_sha256"] != sha(PRIOR_ROUND / "metadata.json")):
        raise ValueError("PRIOR_BUDGET_OR_STATUS_INVALID")
    cases = load_cases()
    ...
    schema = BaselineRates.model_json_schema()
    schema["description"] = schema["description"].replace("두 단가와 추정 근거를", "두 단가를")
    for key in REMOVED:
    ...
    for case_id in CASE_IDS:
        ...
        before = capture_wire(asset, baseline=True)
        after = capture_wire(asset)
        expected = json.loads(json.dumps(before))
        expected["messages"][0]["content"] = without_explanation_instructions(
            expected["messages"][0]["content"],
        )
        wire_schema = expected["response_format"]["json_schema"]["schema"]
        wire_schema["description"] = wire_schema["description"].replace(
            "두 단가와 추정 근거를", "두 단가를",
        )
        for key in REMOVED:
        ...
        requests[case_id] = {
            "outbound_sha256": digest(build_outbound_payload(numeric_request(asset))),
            "wire_sha256": digest(after),
            "wire_bytes": len(json.dumps(after, ensure_ascii=False).encode("utf-8")),
        }
    sources = [
        "apps/core-api/ai/evaluation/savings/numeric.py",
        "apps/core-api/ai/evaluation/savings/numeric_validation.py",
    ...
    return {
        ...
        "prior_attempted_calls": prior_calls,
        "prior_metadata_sha256": sha(PRIOR_ROUND / "metadata.json"),
        "prior_cost_usage_estimate_usd": prior_analysis["cost_with_reported_cache_usd"],
        "user_total_call_limit": USER_TOTAL_CALL_LIMIT,
...
def validate_approval(frozen):
    if (sha(PRIOR_ROUND / "metadata.json") != frozen["prior_metadata_sha256"]
            or frozen["prior_attempted_calls"] + MAX_CALLS > USER_TOTAL_CALL_LIMIT):
        raise ValueError("PRIOR_BUDGET_DRIFT")
    ...
    expected = {
        ...
        "user_total_call_limit": USER_TOTAL_CALL_LIMIT,
        "prior_attempted_calls": frozen["prior_attempted_calls"],
        "candidate": "B", "preflight_sha256": sha(ROUND / "preflight.json"),
    }
...
def analyze(metadata, spec):
    ...
    return {
        ...
        "prior_attempted_calls": metadata["prior_attempted_calls"],
        "cumulative_attempted_calls": metadata["prior_attempted_calls"] + len(calls),
        "unused_user_call_limit": USER_TOTAL_CALL_LIMIT - metadata["prior_attempted_calls"] - len(calls),
        ...
        "cumulative_cost_usage_estimate_usd": str(
            cost + Decimal(metadata["prior_cost_usage_estimate_usd"]),
        ),
        "explanation_source": EXPLANATION_SOURCE,
        ...
        with client_factory(verify_wire) as client:
            for expected in SCHEDULE:
                ...
                try:
                    ...
                    diagnostics = []
                    estimate = accept_numeric_rates(response.output, asset, diagnostics=diagnostics)
                    item.update(estimate=estimate.model_dump(mode="json"), diagnostics=diagnostics,
                                explanation_source=EXPLANATION_SOURCE if estimate.basis else None)
