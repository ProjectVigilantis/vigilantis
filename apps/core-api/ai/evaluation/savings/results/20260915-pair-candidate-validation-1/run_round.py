"""고정한 기존 3호출 후보를 6건 × 10회 검증한다. 기본 동작은 네트워크 없는 사전 확인이다."""
...
def execute(frozen, spec, cases):
    ...
    metadata = {
        "scope": "Approved execution flag: unchanged Luna/low pair candidate, six cases ten times, at most 180 calls, no retries.",
        "trial_version": "v0.7.0", "execution_mode": "three_call_candidate_validation",
        ...
        "credential_preflight": "Previously used main-checkout key; no new credential probe in this round",
        ...
        "stopping_rule": "Continue sixty planned pipelines through quality failures; abort on request, transport or unexpected runner error. Response parse failure skips dependent stages. No call reallocation.",
    }
    raw = {"label": "gpt-5.6-luna/low v0.7.0 three-call",
           "execution_mode": "three_call_candidate_validation", "prompt_version": "v0.7.0",
    ...
    try:
        with OpenAI(api_key=api_key, base_url="https://api.openai.com/v1",
            ...
            for case_id, repeat in SCHEDULE:
                ...
                record["execution_mode"] = "three_call_candidate_validation"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.execute_approved_round:
            ...
            print("PREFLIGHT_OK live_calls=0 columns=1 pipelines=60 maximum_calls=180")
            code = 0
