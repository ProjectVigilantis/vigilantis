...
AI 평가 도구·기준선·공개 실험 요약을 기능별로 관리하는 위치다.
...
기존 구현을 다시 공개하는 경로다. 구현 이동은 #324 범위에 포함하지 않으며 기존 import를 유지한다.
#324의 SecOps 입력·프롬프트·호출 순서·평가 결과 요약은 `secops/`에서 관리한다.
SecOps의 상세 기록·허용된 원자료는 Git에서 제외된 프로젝트 기록 폴더에 두며,
공개 규격·fixture·테스트는 그 기록 없이 사용할 수 있다. 사용법과 재검증은 [SecOps 안내](secops/README.md)를 따른다.
