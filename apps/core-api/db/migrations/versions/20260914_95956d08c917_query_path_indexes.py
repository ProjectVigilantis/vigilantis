...
30일치 부하(run 17,280 · 판정 138,240)에서 두 질의가 24ms · 150ms 였다. 정렬 키와
같은 순서의 복합 인덱스를 두고 repository 를 LATERAL LIMIT 1 로 바꾸면 0.05ms ·
0.08ms — 자산·리전 수에만 비례하고 회차가 쌓여도 늘지 않는다.

metric_summaries.window_end 는 스케줄러가 매 회차 부르는 fresh_ec2_metric_summaries
(window_end >= 기준 시각)가 요약 전체를 훑지 않게 한다(10ms → 0.1ms).
