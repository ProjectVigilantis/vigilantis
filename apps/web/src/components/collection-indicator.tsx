...
const TONE: Record<CollectionStatus, { text: string; dot: string }> = {
  READY: { text: 'text-emerald-400', dot: 'bg-emerald-400' },
  COLLECTING: { text: 'text-amber-400', dot: 'bg-amber-400' },
  PARTIAL: { text: 'text-amber-400', dot: 'bg-amber-400' },
  FAILED: { text: 'text-amber-400', dot: 'bg-amber-400' },
  NOT_COLLECTED: {
...
function statusLabel(status: CollectionStatus): string {
  return COLLECTION_STATUS_LABELS[status]?.label ?? '연결됨';
}
...
export function CollectionIndicator() {
  ...
  const label = statusLabel(env.collection_status);
  ...
  return (
    <span className="flex shrink-0 items-center gap-2 rounded-md border px-2.5 py-1 text-xs whitespace-nowrap">
      ...
      {name ? (
        <>
          <span>{name}</span>
          <span className={cn('border-l pl-2', tone.text)}>{label}</span>
        </>
      ) : (
